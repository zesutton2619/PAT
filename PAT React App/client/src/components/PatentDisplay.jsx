...
import placeholderPDF from './placeholder-pdf.pdf';
import PDFplaceholder from "./PDFplaceholder"; // Import the placeholder PDF file
...
const Display = ({ patents }) => {
    ...
    return (
        <div className="flex bg-white shadow-lg mx-5 rounded-sm border border-gray-300 px-8 p-5 w-full mt-7">
            <div>
                <div className="flex">
                    <h1 className="text-3xl font-bold text-gray-700 border-gray-600 mb-2 text-center mx-auto mb-6">Compared
                        Patents</h1>
                </div>
                     ...
                     style={{gridAutoCols: '1fr'}}>
                    {
                        patents.length === 0 ? (
                            yourArray.map((item, index) => (
                                <div
                                    className="patent-card p-3 border-gray-800 border-4 rounded-lg transform transition duration-500 ease-in-out hover:-translate-y-1 hover:scale-105"
                                    style={{minWidth: '100%', maxWidth: 'calc(100% - 20px)', margin: '0 auto'}}>
                                    <h3 className="text-2xl font-bold border-gray-700 mb-2 text-center">No Patents
                                        Found</h3>
