...
directory_to_watch = os.path.abspath("C:\\Users\\kskos\\PycharmProjects\\PATFinal\\Patents\\Utility Patents")  # Change this to your directory


# Handler for directory events
...
def update_files_json():
    ...
    try:
        ...
        # List comprehension to get files instead of directories
        files = [name for name in os.listdir(directory_to_watch)
                 if os.path.isfile(os.path.join(directory_to_watch, name))]
        files.sort()  # Sort files alphabetically; adjust as needed

        print("Writing Directory into file")
        with open('files.json', 'w') as f:  # Change the output file name to files.json
